...
public abstract class BaseEntityService<T> {
	...
	public List<T> getAll(MultivaluedMap<String, String> queryParameters) {
		...
		/*
		 * We have to know where to select data from. Known as "Query Root"
		 */
		Root<T> root = criteriaQuery.from(entityClass);

		/*
		 * Predicate is some form of query expression that evaluates to either
		 * true or false It is used later in WHERE clause (see:
		 * .where(predicates))
		 */
		...
		/*
		 * TypedQuery interface was introduced in JPA2.
		 * 
		 * It is much easier to run queries and process the results in a type
		 * safe manner when using the TypedQuery interface.
		 * 
		 * Use it instead of Query interface then.
		 */
